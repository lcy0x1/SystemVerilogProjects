`ifndef UVM_TRANSPOSE_MONITOR
`define UVM_TRANSPOSE_MONITOR

class TransposeInputMonitor #(W=7) extends uvm_monitor;

    `uvm_component_param_utils(TransposeInputMonitor#(W))

    virtual transpose_bus #(W) vif;

    uvm_analysis_port #(TransposeInputTransaction#(W)) analysis_port;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            @(posedge vif.clk);
            if(!vif.en) continue;
            tr = TransposeInputTransaction#(W)::type_id::create("tr", this);
            tr.transpose = vif.do_transpose;
...
class TransposeOutputMonitor #(W=7) extends uvm_monitor;

    `uvm_component_param_utils(TransposeOutputMonitor#(W))

    virtual transpose_bus #(W) vif;

    uvm_analysis_port #(TransposeOutputTransaction#(W)) analysis_port;
    ...
    virtual task run_phase(uvm_phase phase);
        TransposeOutputTransaction #(W) tr;
        int j;
        forever begin
            @(posedge vif.clk);
            if(!vif.valid) continue;
		    phase.raise_objection(this, "Start recording");
            tr = TransposeOutputTransaction#(W)::type_id::create("tr", this);
