...
wire do_clock;
assign do_clock = en || en_delay || v_count > 0 && v_count < 24;
...
always @(posedge clk) begin
    if(reset)begin
        ...
        en_delay <= en ? 1 : 0;
        transpose_en <= en ? do_transpose : v_count>0 ? transpose_en : 0; 
        ...
        valid <= v_count > 16;
        v_in <= do_clock ? v_count[0] == 0 ? 8'h11 << v_count[2:1] : 0 : 0;
        clear_in <= do_clock ? 1 << v_count[2:0] : 0;
        out_mult_clear <= out_mult_clear_pre;
    end
end

assign z_out[0] = transpose_en ? y_out[0] : x_delay_out[0][31:0];
assign z_out[1] = transpose_en ? y_out[1] : x_delay_out[1][31:0];
assign z_out[2] = transpose_en ? y_out[2] : x_delay_out[2][31:0];
assign z_out[3] = transpose_en ? y_out[3] : x_delay_out[3][31:0];
assign z_out[4] = transpose_en ? y_out[4] : x_delay_out[4][31:0];
assign z_out[5] = transpose_en ? y_out[5] : x_delay_out[5][31:0];
assign z_out[6] = transpose_en ? y_out[6] : x_delay_out[6][31:0];
assign z_out[7] = transpose_en ? y_out[7] : x_delay_out[7][31:0];
